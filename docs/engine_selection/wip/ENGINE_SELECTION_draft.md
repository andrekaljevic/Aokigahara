# Engine selection: decision record (draft)

Date: 23 September 2026. Status: draft for the owner's review. Branch of record:
`claude/aokigahara-engine-selection-tqyqx6`. The owner has excluded Unreal Engine 5, so it was not scored.
...
three. A Godot render of this repository's real data already meets 3 of the 5 measured lighting targets, and it was
rendered headless in the cloud without the Mac.
...
**Hardware.** Model MC7X4 is sold as the 16 GB / 256 GB M2 Air with an 8-core GPU [21]:
...
Running `tools/lighting_stats.py` on frames g–j gives these ranges [1]:

| Metric | Target range |
|---|---|
| Sunfleck fraction | 0.07–0.16 |
| Log-average luminance (exposure key) | 0.003–0.008 |
| Black fraction | 0.17–0.43 |
| Highlight B/R (warm sunlit patches) | 0.29–0.42 |
| Dynamic range | 8.5–10.2 stops |
...
headless, in a cloud container with no GPU [2]. **Bold** means in range.

| Render (under `docs/engine_selection/prototypes/`) | log-avg | sunfleck | black | highlight B/R | stops | Met |
|---|---|---|---|---|---|---|
| Godot, SDFGI (`godot/renders/godot_sdfgi.jpg`) | **0.0056** | **0.157** | **0.27** | 0.70 | 8.18 | 3/5 |
| Godot, SDFGI + canopy map, stronger sun (`godot/renders/godot_sdfgi_canopy_sun6.jpg`) | **0.0078** | 0.163 | **0.20** | 0.66 | **8.60** | 3/5 |
| Godot, no SDFGI, baked canopy map (`godot/renders/godot_canopy.jpg`) | **0.0064** | **0.124** | **0.18** | 0.70 | 6.75 | 3/5 |
| Godot, neither (`godot/renders/godot_nosdfgi.jpg`) | 0.0106 | **0.083** | 0.05 | 0.70 | 6.50 | 1/5 |
| three.js, plain + canopy map (`webgpu/renders/t4_off_tuned.jpg`) | **0.0036** | 0.34 | 0.51 | 0.48 | 12.8 | 1/5 |
| three.js, SSGI (`webgpu/renders/t5_ssgi_tuned_BROKEN.jpg`) | 0.099 | 0.001 | 0.00 | 0.52 | 5.9 | 0/5 |
...
2. **Gate.** On the four matched cameras, at least 4 of 5 metrics must be in range, and warm highlights must be one of
   them. The sky must not clip: `clipped_fraction` below 0.5%.
...
**P1 Godot** won under all three lenses. It is the only path with the measured lighting signature of frames 6–10 on
...
integrated GPU is unmeasured [7], and a Metal freeze reproduced on an M2 Air is still open [11].
...
- **Godot 4.7.2, Forward+ on lavapipe under `xvfb-run` with `--write-movie` [30]:**
  ...
  - renders are in `docs/engine_selection/prototypes/godot/renders/`, and PNG originals (temporary) are in
    `/tmp/claude-0/-home-user-Aokigahara/f3a7dee1-ddd5-5966-b7a1-ff45901003d9/scratchpad/proto/godot/out/`.
- **three.js on SwiftShader WebGPU:** about 20 s per frame. Renders are in
  `docs/engine_selection/prototypes/webgpu/renders/`; PNG originals are in `.../scratchpad/proto/web/out/`.
...
- **Sessions:** one scoped single-agent session per step. Use plan mode first, `/clear` between steps, and Sonnet for
  routine work. Multi-agent runs cost about 15× the tokens of a chat, so keep them for one-off research [22].
...
2. Attach a 1 TB+ APFS NVMe SSD and move the survey folder onto it.
...
- the section 4 lighting gate passes on the six fixed cameras in `harness/cams_ue58.json`;
