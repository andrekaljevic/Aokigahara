...
asset_packs/       Photo-derived 4K texture, sky, lighting and mood packs (see asset_packs/README.md)
```
...
`asset_packs/` holds one pack per reference photograph: seamless 4096² PBR sets for bark, greenery
and ground in the runtime's own map layout, a 4096 × 2048 sky dome fitted with the viewer's Sky
shader, a daylight preset in the exact `PRESETS` shape, and a palette, tonal record and `.cube`
grade — plus a contact sheet and a manifest that separates what was measured from what was
assumed. They are generated by `construction_code_v2/gen_asset_packs.py` and are **not loaded by
the runtime yet**. None of the photographs was taken at Aokigahara; one pack (a watermarked stock
preview) carries measurements only, no pixels. Read `asset_packs/README.md` before using them.
...
*.npz *.zip *.7z *.bin *.pdf`, the runtime texture sets under `viewer/assets/materials*/`, the
photo-derived packs under `asset_packs/`, and the render evidence under `renders/`. Small categorical grids (`*.u8`, all under 300 KB) and every
