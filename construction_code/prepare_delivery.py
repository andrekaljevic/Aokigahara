...
ROOT=Path('/workspace/scratch/f25aff235357');SITE=Path(__file__).parent;A=SITE/'dist/assets';O=ROOT/'recreation_output';O.mkdir(exist_ok=True)
raw=A/'Aokigahara_Cave_Corridor.glb';assert raw.exists();shutil.copy2(raw,O/raw.name)
for name in ['Aokigahara_Surface_Terrain.glb','Aokigahara_Regional_Terrain.glb','Aokigahara_Instanced_Forest.glb']:shutil.copy2(A/name,O/name)
readme='''# Aokigahara: exportable 3D reconstruction
...
'''
(O/'Aokigahara_3D_README.md').write_text(readme);shutil.copy2(A/'provenance.json',O/'Aokigahara_3D_Provenance.json')
# Prepare a resumable small-part download for static hosting's individual-asset limit.
zip_path=O/'Aokigahara_Cave_Corridor.zip'
with zipfile.ZipFile(zip_path,'w',zipfile.ZIP_DEFLATED,compresslevel=6) as z:
 z.write(raw,raw.name);z.write(O/'Aokigahara_3D_README.md','README.md');z.write(A/'provenance.json','provenance.json')
for old in A.glob('corridor-download-*.bin'):old.unlink()
parts=[]
with zip_path.open('rb') as f:
 ...
 while True:
  ...
  if not data:break
  name=f'corridor-download-{i:02d}.bin';(A/name).write_bytes(data);parts.append({'file':name,'bytes':len(data),'sha256':hashlib.sha256(data).hexdigest()});i+=1
(A/'corridor-download.json').write_text(json.dumps({'fileName':zip_path.name,'mimeType':'application/zip','parts':parts,'bytes':zip_path.stat().st_size},indent=2))
raw.unlink()
old=A/'Aokigahara_Cave_Corridor.zip'
if old.exists():old.unlink()
for p in A.glob('*_MR.png'):p.unlink()
print('Corridor prepared',zip_path.stat().st_size,'bytes;',len(parts),'download parts')
